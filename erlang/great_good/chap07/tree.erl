-module(tree).
-export([empty/0, insert/3, lookup/2, test/0]).
...
test() ->
    ...
    io:format("find mail of Anita Bath: ~p~n", [tree:lookup("Anita Bath", Addresses)]),
    io:format("find mail of Jacques Requin: ~p~n", [tree:lookup("Jacques Requin", Addresses)]).
